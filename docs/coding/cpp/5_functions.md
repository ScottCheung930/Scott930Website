...
### 按值传参
即使按值传递, 对于容器C++也有一些自动优化, 因为容器通常定义了移动/拷贝, 编译器会自懂使用.
...
事实上, 除了上述解释所涉及的, 上述代码的优化还包括对象被同类型prvalue初始化时强制**copy elision**的作用(C++ 17之后): 在C++17之前, 返回时理应先在函数调用处testVector(datas)处构造一个临时对象ret, 而后再使用ret**移动构造**给datas1(这里是初始化datas1, 所以是移动构造不是移动复制). 而C++17之后, 对象被同类型prvalue初始化时将会强制copy elision, 其作用是不构造临时对象ret, 而直接构造datas1, 节省了一次移动构造. 如果在自定义类的拷贝和移动构造函数中写输出, 并分别使用```g++ -std=c++11```和```g++ -std=c++17```编译, 就可以观察到结果. 不过忽略copy elision并不影响解释地址相同的现象, 只是两次移动构造(datas -> ret-> datas1)和一次移动构造(datas->datas1)的区别.

另外, 这里的例子是完全不涉及NRVO(命名返回值优化)的, 因为返回的是形参, 返回形参不适用于NRVO. 

(关于Copy elision和NRVO, 详见[Copy elision 和 NRVO](8_cpp_heap_memory.md)).
