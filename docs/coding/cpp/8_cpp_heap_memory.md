...
???+ Note "value categorr: 左值, 纯右值, 将亡值"
    首先需要明确: 
    1. "value category"是独立于"type"的属性; 
    2. "value category"是"表达式的类别"而非值的类别("The approach we take to provide guaranteed copy elision is to tweak the definition of C++'s 'glvalue' and 'prvalue' [value categories (which, counterintuitively, categorize expressions, not values).](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2015/p0135r0.html)")
