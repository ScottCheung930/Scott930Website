...
1. log.cpp中定义log_stream时直接写```std::ostream log_stream(cout.rdbuf())```可能有静态初始化顺序问题(详见**面向对象-类成员变量的初始化**). 静态初始化顺序问题是指不同编译单元间的静态初始化顺序是不明确的. 

    当存在跨编译单元的依赖(```extern```)时, 就有可能出现用还没初始化的全局对象去初始化另一个全局对象. 
